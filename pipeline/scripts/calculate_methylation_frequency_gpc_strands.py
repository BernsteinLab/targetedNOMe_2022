...
def make_key(c, s, e, strand):
    return c + ":" + str(s) + ":" + str(e) + ":" + strand
...
for record in csv_reader:
    ...
    if args.split_groups and record['start'] != record['end']:
        ...
        while gc_pos != -1:
            key = make_key(c, s + gc_pos - first_gc_pos, s + gc_pos - first_gc_pos, strand)
...
for key in sorted_keys:
    if sites[key].called_sites > 0:
        (c, s, e, strand) = key.split(":")
        f = float(sites[key].called_sites_methylated) / sites[key].called_sites
        print("%s\t%s\t%s\t.\t%.3f\t%s\t%d\t%d" % (c, int(s)+1, int(e)+2, f, strand, sites[key].called_sites, sites[key].called_sites_methylated))