"""
Script to convert bam files so they can be viewed in IGV

Adapted from: https://github.com/isaclee/nanoNOMe/blob/master/scripts/convertBam.py
...
def convertBam(bampath,ref,cfunc,cpgpath,gpcpath,verbose,q) :
    if verbose : print("reading {} from bam file".format(''),file=sys.stderr)
    # Load in reference
    genome_dict = dict()
    if ref :
    ...
    if verbose : print("{} reads in {}".format(len(bam_entries),''),file=sys.stderr)
    if len(bam_entries) == 0 : return
    if verbose : print("reading {} from cpg data".format(''),file=sys.stderr)
    cpg_calldict = read_tabix(cpgpath)
    if verbose : print("reading {} from gpc data".format(''),file=sys.stderr)
    try: gpc_calldict = read_tabix(gpcpath)
    except TypeError : gpc_calldict = cpg_calldict # no gpc provided, repace with cpg for quick fix
    if verbose : print("converting bams in {}".format(''),file=sys.stderr)
    i = 0
    ...
    if verbose : print("converted {} bam entries in {}".format(i,''),file=sys.stderr)
...
def main() :
    ...
    jobs = [ pool.apply_async(convertBam,
        args = (args.bam,args.fasta,converter,args.cpg,args.gpc,args.verbose,q))
        ]
