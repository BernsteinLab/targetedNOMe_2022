...
for record in csv_reader:
    ...
    if args.split_groups and record['start'] != record['end']:
        ...
        while cg_pos != -1:
            key = make_key(c, s + cg_pos - first_cg_pos, s + cg_pos - first_cg_pos, strand)
...
for key in sorted_keys:
    if sites[key].called_sites > 0:
        ...
        print("%s\t%s\t%s\t.\t%.3f\t%s\t%d\t%d" % (c, int(s), int(e)+1, f, strand, sites[key].called_sites, sites[key].called_sites_methylated))
